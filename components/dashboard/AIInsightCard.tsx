...
import { dashboardInsight } from "@/lib/mock/ai";
import { StatusPill } from "@/components/ui/Badge";

export function AIInsightCard() {
  const { tag, title, insight, why, action, caveat } = dashboardInsight;
  return (
    <div className="relative overflow-hidden rounded-2xl border border-gold/25 bg-gradient-to-b from-goldsoft/60 to-surface p-5">
      ...
      <div className="relative">
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="rounded-xl border border-edge bg-surface/80 p-3.5">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-gold">Suggested action</p>
            <p className="mt-1.5 text-[12.5px] leading-relaxed text-secondary">{action}</p>
          </div>
        </div>

        <p className="mt-4 border-t border-edge/70 pt-3 text-[10.5px] leading-relaxed text-muted">
          {caveat}
        </p>
      </div>
    </div>
